...
def avg_filter(image):
    ...
    while i < len(image)-1:
        ...
        while j < len(image[0])-1:
            avim[i][j] = (image[i-1][j-1]+image[i-1][j]+image[i-1][j+1]+image[i][j-1] +
                          image[i][j]+image[i][j+1]+image[i+1][j-1] + image[i+1][j]+image[i+1][j+1])//9
            j += 1
        i += 1
    return avim
...
def edge_det(image):
    ...
    for i in range(1, len(x1)-1):
        for j in range(1, len(x1[0])-1):
            hdif[i][j] = x1[i-1][j-1] - x1[i-1][j+1] + \
                (2*(x1[i][j-1]-x1[i][j+1])) + x1[i+1][j-1] - x1[i+1][j+1]
            vdif[i][j] = x1[i-1][j-1] - x1[i+1][j-1] + \
                (2*(x1[i-1][j]-x1[i+1][j])) + x1[i-1][j+1] - x1[i+1][j+1]
...
def MinEnergyPath(im):
    ...
    for i in range(1, len(x)):
        for j in range(1, len(x[0])-1):
            MinEnergy[i][j] = x[i][j] + \
                min(MinEnergy[i-1][j-1], MinEnergy[i-1]
                    [j], MinEnergy[i-1][j+1])
        MinEnergy[i][0] = x[i][0] + min(MinEnergy[i-1][0], MinEnergy[i-1][1])
        MinEnergy[i][-1] = MinEnergy[i][-1] = x[i][-1] + \
            min(MinEnergy[i-1][-2], MinEnergy[i-1][-1])
    minen = min(MinEnergy[-1])

    ml = [[]for i in range(len(MinEnergy))]
    ...
    while i > 0:
        for j in ml[i]:
            if j == 0:
                if len(MinEnergy[i]) > 1:
                    m = min(MinEnergy[i-1][j], MinEnergy[i-1][j+1])
                    if MinEnergy[i-1][j] == m:
                        final[i-1][j] = 255
                        if j not in ml[i-1]:
                            ml[i-1].append(j)
                    if MinEnergy[i-1][j+1] == m:
                        final[i-1][j+1] = 255
                        if j+1 not in ml[i-1]:
                            ml[i-1].append(j+1)
                else:
                    final[i-1][j] = 255
                    ml[i-1].append(j)
            elif 0 < j < len(MinEnergy)-1:
                m = min(MinEnergy[i-1][j-1], MinEnergy[i-1]
                        [j], MinEnergy[i-1][j+1])
                if MinEnergy[i-1][j-1] == m:
                    final[i-1][j-1] = 255
                    if j-1 not in ml[i-1]:
